...
if __name__ == '__main__':
    ...
    add_filehandler(logger, 'search_test.log')
    ...
    reqs = []
    for i in range(cv_num):
        reqs.append(train_model.remote(copy.deepcopy(copied_c), args.dataroot, C.get()['aug'], args.cv_ratio, i, save_path=paths[i], skip_exist=True))
    ...
    for _ in range(1):  # run multiple times.
        for cv_fold in range(cv_num):
            ...
            register_trainable(name, lambda config, checkpoint_dir: eval_tta(copy.deepcopy(copied_c), config, checkpoint_dir))
            # algo = HyperOptSearch(space, max_concurrent=4*20, reward_attr=reward_attr)
            # algo = HyperOptSearch(space, metric=reward_attr, max_concurrent=4*20)
            ...
            results = run_experiments(exp_config, scheduler=None, verbose=0, resume=args.resume, raise_on_failed_trial=False)
            print()
